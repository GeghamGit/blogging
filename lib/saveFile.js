...
exports.saveFile = async (image, imgConfPath, res, next) => {
  try {
    ...
    if (imgConfPath === 'user'){
      ...
      //image base64 to original path with unique name
      const savedImageName = await imgSync(image, `${__dirname}/${conf.media.user_original_dir}`, imageName);
      fullImageName = `${imageName}${extname(savedImageName)}`;
      ...
      //resize original image to cropped and minimum sizes
      croppImage(savedImageName, savePathCropped, 510);
      croppImage(savedImageName, savePathMin, 210);
    };
